...
export const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<'ol'>>(
  ({ className, ...props }, ref) => (
    <ol
      ref={ref}
      className={cn(
        'flex flex-wrap items-center gap-1.5 text-sm text-[var(--tei-muted)] sm:gap-2.5',
        className,
      )}
      {...props}
    />
...
>(({ asChild, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      className={cn('transition-colors hover:text-[var(--tei-text-strong)]', className)}
      {...props}
    />
  );
});
...
export const BreadcrumbPage = React.forwardRef<HTMLSpanElement, React.ComponentPropsWithoutRef<'span'>>(
  ({ className, ...props }, ref) => (
    <span
      ...
      aria-disabled="true"
      aria-current="page"
      className={cn('font-normal text-[var(--tei-text-strong)]', className)}
      {...props}
    />
  ),
);
